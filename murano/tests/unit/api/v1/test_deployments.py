...
class TestDeploymentsApi(tb.ControllerTest, tb.MuranoApiTestCase):
	...
	self.expect_policy_check('list_deployments', {'environment_id': ENVIRONMENT_ID})
	result = self.deployments_controller.index(request, ENVIRONMENT_ID)
	self.assertEquals([], result['deployments'])
	...
	deploy_id = '1'
		...
		{'deployment_id': deploy_id, 'environment_id': ENVIRONMENT_ID})
        self.assertRaises(exc.HTTPNotFound, self.deployments_controller.statuses,request, ENVIRONMENT_ID, deploy_id)
	...
	deploy_id =  result['deployments'][0]['id']
        ...
        self.expect_policy_check('statuses_deployments',
                {'deployment_id': deploy_id, 'environment_id': ENVIRONMENT_ID})
        result = self.deployments_controller.statuses(
				request, ENVIRONMENT_ID, deploy_id)
	self.assertNotEquals(result['reports'], [])
	...
	self.expect_policy_check('statuses_deployments',
        ...
        result = self.deployments_controller.statuses(
                                request, ENVIRONMENT_ID, deploy_id)
        self.assertEquals(result['reports'], [])

	self.expect_policy_check('statuses_deployments',
                {'deployment_id': deploy_id, 'environment_id': '12'})
	self.assertRaises(exc.HTTPBadRequest, self.deployments_controller.statuses, request, "12", deploy_id)
	...
	self.assertEquals('success',result.state)
	...
	filter_by_result.count.return_value = 1 
        query_result.filter_by.return_value = filter_by_result
	result = deployments.set_dep_state(deployment, unit)    
        self.assertEquals('completed_w_errors',result.state)

	"""
	result = deployments.set_dep_state(deployment, unit)    
        self.assertEquals('completed_w_warnings',result.state)
	"""
	...
	result = deployments.set_dep_state(deployment, unit)    
        self.assertEquals('running',result.state)

	filter_by_result.count.return_value = 1
        query_result.filter_by.return_value = filter_by_result
	result = deployments.set_dep_state(deployment, unit)    
        self.assertEquals('running_w_errors',result.state)

	"""
	result = deployments.set_dep_state(deployment, unit)    
        self.assertEquals('running_w_warning',result.state)
	"""
